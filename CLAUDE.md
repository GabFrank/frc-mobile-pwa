...
⚠️ **La marcación con rostro necesita un central con la migración `V216.5`.** Desde el kiosco 1:N y desde la marcación personal viajan `metodoRegistro`, `similitudFacial` y `margenSegundoCandidato`; contra un central que no los conozca **la mutation falla entera y no se registra ninguna marcación**, no solo los campos nuevos. Las dos mitades se publican juntas. Y la migración **tiene que correr también en las filiales**: `administrativo.marcacion` se replica en las dos direcciones, así que una columna que exista solo en el central corta la replicación hacia la filial que no la tenga. Ver [`docs/modulos/marcacion.md`](docs/modulos/marcacion.md).
